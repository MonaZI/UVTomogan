...
from train_abstract_2d import ImageClass
            ...
            for iter, real_meas in enumerate(self.dataloader):
                ...
                if (self.iteration%self.args.n_disc==0) and (self.iteration>0):
                    ...
                    if not self.args.pdf_known:
                        ...
                        #torch.nn.utils.clip_grad_norm_(self.p, 1)
                        #self.optim_pdf.step()
                ...
                if (self.iteration%self.args.iter_log==0) and (self.iteration>0):
                    ...
                    snr = SNR(image.detach().cpu(), self.image_true.cpu())
                    axes[0, 0].set_title('iter={0:1.1f}, snr={1:2.2f}'.format(self.iteration, snr))
                    ...
                    if not self.args.pdf_known:
                        axes[0, 2].plot(np.linspace(0, 1*np.pi, self.args.angle_disc), self.args.pdf_vec, label='pdf-gt')
